    ...
    <ModalShell id="add-client-title" title="Add Client" onClose={onClose}>
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          ...
          <Field label="Email Address" type="email" id="add-email" placeholder="client@example.com" value={email} onChange={e => setEmail(e.target.value)} required />
        ...
        <div className="flex gap-3">
          ...
          <button type="submit" disabled={isLoading} className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-2xl bg-foreground text-background text-sm font-semibold hover:opacity-80 transition-opacity disabled:opacity-50">
            {isLoading ? <Loader2 className="animate-spin" size={16} /> : "Add Client"}
          </button>
        </div>
      </form>
    </ModalShell>
