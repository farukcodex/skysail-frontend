...
export function ConfirmBlockModal({ vendor, onClose, onConfirm }: ConfirmBlockModalProps) {
  ...
  return (
    <ModalShell id="confirm-block-title" title={isBlocked ? "Unblock Vendor" : "Block Vendor"} onClose={onClose}>
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-4 p-4 rounded-xl bg-secondary/50">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            Are you sure you want to {isBlocked ? 'unblock' : 'block'} <strong className="text-foreground">{vendor.firstName} {vendor.lastName}</strong>? 
            {isBlocked 
