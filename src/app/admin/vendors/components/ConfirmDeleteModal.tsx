...
export function ConfirmDeleteModal({ vendor, onClose, onConfirm }: ConfirmDeleteModalProps) {
  ...
  return (
    <ModalShell id="confirm-delete-title" title="Delete Vendor" onClose={onClose}>
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-4 p-4 rounded-xl bg-red-500/10 border border-red-500/20">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            Are you sure you want to delete <strong className="text-foreground">{vendor.firstName} {vendor.lastName}</strong>? 
        ...
        <div className="flex gap-3">
          ...
          >
            {isLoading ? <Loader2 className="animate-spin" size={16} /> : "Yes, Delete Vendor"}
          </button>
        </div>
      </div>
    </ModalShell>
